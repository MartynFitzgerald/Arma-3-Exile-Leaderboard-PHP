...
while ($amountOfData < 10) {
	$sql_weapon1 = "SELECT weaponused FROM player_stats WHERE killer='$uid' GROUP BY weaponused ORDER BY count(*) DESC LIMIT 1 OFFSET $amountOfData;";
	...
	if ($sql_result_weapon1->num_rows > 0) {
		...
		if($row = $sql_result_weapon1->fetch_assoc()) {
			...
			$amount = "SELECT COUNT(weaponused) FROM player_stats WHERE weaponused='$resultweapon' AND  killer='$uid';";
